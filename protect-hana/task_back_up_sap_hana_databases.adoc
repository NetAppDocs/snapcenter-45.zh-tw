...
* 如果您想要在一或多個租戶資料庫關閉時建立檔案型備份、請使用Set-SmConfigSettings Cmdlet在HANA內容檔中將允許檔案型備份參數設定為*是*。
+
您可以執行Get-Help _command_name_來取得可搭配Cmdlet使用之參數及其說明的相關資訊。或者、您也可以參閱 https://library.netapp.com/ecm/ecm_download_file/ECMLP2877143["《軟件指令程式參考指南》SnapCenter"]
...
. 按一下您要備份的資源。
. 在「*資源*」頁面上、選取*使用Snapshot COPU*的自訂名稱格式、然後輸入您要用於Snapshot複本名稱的自訂名稱格式。
+
